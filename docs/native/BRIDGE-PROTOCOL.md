...
{ "prepared": true, "sampleRate": 48000, "blockSize": 64, "outputs": 2, "inputs": 2, "playing": false,
  ...
  "mixer": { "active": [0, 1, 2], "silent": [2], "strips": { "0": [0.5, 0.5, 0.5, 0.5, 0.35, 0.35, 1.0], "1": [0.5, 0.5, 0.5, 0.5, 0.35, 0.35, 1.0], "2": [0.1, 0.1, 0, 0, 0.07, 0, 0] },
             "rejected": 0, "orderValid": true, "mainOut": 0, "bassStrip": 7, "clickStrip": 12 } }
```
`mixer` (4.1): the LIVE strips only — `active` (indices; 0 = the master), `silent` (mute / the solo law, the target),
`strips["<index>"]` = `[preL, preR, postL, postR, rmsPre, rmsPost, gain]` = the strip's input peak per channel, its output
peak (post fader/mute/pan), RMS of both (pooled) over the 4096-sample window (the TS peak-meter worklet's), and the
smoothed fader × mute gain at the block end (1 = unity; the probe asserts 0 / 1 after a move); `rejected` = lifetime
routes the cycle guard refused, `orderValid` (false = the fallback order runs — cannot happen with the guard),
