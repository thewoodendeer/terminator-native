...
{ "prepared": true, "sampleRate": 48000, "blockSize": 64, "outputs": 2, "inputs": 2, "playing": false,
  ...
  "mixer": { "active": [0, 1, 2], "silent": [2], "strips": { "0": [0.5, 0.5, 0.5, 0.5, 0.35, 0.35, 1.0, 0], "1": [0.5, 0.5, 0.5, 0.5, 0.35, 0.35, 1.0, 2], "2": [0.1, 0.1, 0, 0, 0.07, 0, 0, 0] },
             "rejected": 0, "fxRejected": 0, "orderValid": true, "mainOut": 0, "bassStrip": 7, "clickStrip": 12 } }
```
...
refusals (full / dead / pool exhausted), `orderValid` (false = the fallback order runs — cannot happen with the guard),
`mainOut` = the master's hardware pair, `bassStrip` / `clickStrip` = `setSourceStrip` (−1 = direct).
