...
| Build | CMake + Ninja (brew install ninja), JUCE 9.0.1 via FetchContent pinned to the tag (nothing downloaded by hand; the osx zip is only the Projucer GUI, unused) | Same CMake on Mac/Win; GitHub Actions builds both (macos-14 arm64 + macos-13 Intel runner, windows-latest). |
...
9.1 Sparkle/WinSparkle + R2 `terminator-native/` feeds + RELEASE-CYCLES-NATIVE.md; 9.1b **Upgrade the JUCE licence to Indie before the first paid release** (Starter is free only under $20k revenue; the upgrade is a key + agreement, no code change).
