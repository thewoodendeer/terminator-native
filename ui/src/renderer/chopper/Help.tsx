    ...
    body: [
      ...
      { p: 'Right-click any pad — or press its □ corner — for its menu. Every pad is a little sampler of its own: it can hold its own audio, not just a chop of the main sample.' },
      {
        k: [
          ...
          ['🔁 TERMINATOR OUTPUT (resample)', 'in RECORD SAMPLE\'s INPUT list under TERMINATOR: record what Terminator itself is playing — the final mix, exactly what comes out of the speakers, mixer and all (in this app the ENGINE makes that take, off its own output, so it is the real mix and the count-in clicks are not in it). Press REC, hit pads, run the sequencer, ride faders, then STOP: the take lands on the next empty pad (or the pad you aimed RECORD INTO at) as a fresh sample you can chop again. It is tapped before the speakers and never routed back, so it cannot feed back'],
          ...
          ...(inNativeShell() ? [['⇱ DRAG OUT', 'in the pad menu (right-click a pad, or its \u25a1 button): PRESS AND DRAG that item straight into Finder, Ableton, Logic — anywhere that takes a file. What lands is what the pad PLAYS — the chop with its pitch, reverse and attack — as a 24-bit WAV. A plain click does nothing: it is a drag, not a button. The file is written the moment the menu opens, so the drag starts instantly'] as [string, string]] : []),
    ...
    body: [
      ...
      { p: 'Every strip carries four send knobs, S1 – S4, each feeding one of the four SEND RETURN strips at the right of the desk. The idea: put the reverb or delay ON THE RETURN (it is locked 100 % wet there), then turn up S1 on every channel that should sit in that room — one reverb, many channels, one fader to pull it all back. Drag a send knob up to feed it, double-click to take it out. A return nothing feeds stays silent and is left out of Trackouts; the SENDS button collapses the four returns to name + fader when you want the space back.' },
      {
        k: [
          ...
          ['CHANNEL', 'the SSL 4000 G channel strip — filters, a four-band EQ and a dynamics section in one box, in the order the desk has them. The switch that matters is CURVE. E is the 1981 black-knob EQ: its Q is constant, so a boost is the same shape whether it is 3 dB or 15, which makes it a surgical tool. G is the 1987 brown-knob one, where the Q WIDENS as the gain comes back towards zero and tightens as you push it — small moves are broad and flattering, big moves are precise. That is the entire reason both desks are famous, and it is why G is the default here. The dynamics section is a compressor and a gate/expander sharing one detector (which is why they never fight each other), and DYN can put them BEFORE the EQ instead of after it, exactly like the routing button on the desk. Engine effect, so it is in your Master Mixdown and Trackouts'],
          ['EQ 6', 'six independent bands, each of which can be any shape you need: a bell, a low or high shelf, a low or high cut, a notch, or a TILT that leans the whole spectrum at once. The cuts are the real thing — cascaded Butterworth sections, 12 / 24 / 36 / 48 / 72 / 96 dB per octave — not one filter with its resonance wound up pretending to be steep. A 96 dB/oct low cut takes everything under it away and leaves what is above it exactly where it was. A band set to OFF costs nothing at all, so leaving five of them off is free. Want mid/side EQ? Put two of these on the strip and set one ROUTE selector to MID and the other to SIDE. Engine effect, so it is in your Master Mixdown and Trackouts'],
      ...
      { h: 'GROUPS AND BUSES' },
      { p: 'Select several mixer strips (drag a box across them, or click with shift) and press GROUP. A new bus appears, every selected strip feeds it instead of going straight to the master, and the bus goes to the master — so your drums, or your chops, get ONE fader, ONE insert chain and one place to put the glue compressor. Pull the bus fader and the whole group comes down together. Buses are saved with the project and are rendered into your Master Mixdown and Trackouts, so a bounce has the same routing as playback. The engine owns the routing and refuses to make a loop, so you cannot accidentally feed a bus back into itself.' },
      { h: 'MID / SIDE ON ANY EFFECT' },
      { p: 'Every device panel has a small ROUTE selector next to its name: STEREO, MID, SIDE, LEFT, RIGHT. It is not one of the device\'s controls — it decides what the device is POINTED AT, and it works on all of them, the stock ones and the engine ones alike. MID is the centre of the image, where your vocal, kick, snare and bass live. SIDE is everything that is not the centre. So you can compress the middle without squashing the width around it, brighten the sides without making the vocal harsh, or de-ess one channel only. STEREO is the default and is bit-exact — the split does not run at all unless you ask for it.' },
      { tip: 'A device that takes TIME to work (COMP, the drives, VINYL, LIMITER) only delays the half it processes, so the other half would comb-filter against it. The engine delays the untouched half to match automatically. There are eight of those compensators; if you somehow use all eight, the ninth slot stays on STEREO rather than sounding subtly wrong.' },
      ...
      { p: 'PLUGINS IN THE MIXER (Terminator 3, desktop): any insert slot can be one of YOUR plugins. ＋ INSERT FX → PLUGIN, then pick it from the list (it shows every effect Preferences → PLUGINS has found), press EDITOR for the plugin\u2019s own window, and WET blends it against the dry signal like any other insert — so a plugin can sit in parallel without a second strip. The plugin runs in Terminator\u2019s own engine, in the chain where you put it, and its latency is compensated with everything else. The choice and the plugin\u2019s own settings travel with the chain: copy the slot to another strip, save the project, open it tomorrow — the same plugin comes back the way you left it.' },
      { p: 'YOUR SYNTHS TOO (Terminator 3, desktop): an INSTRUMENT plugin picked in a slot does not process that channel — it PLAYS into it, like a synth patched into the desk. Pick it the same way (＋ INSERT FX → PLUGIN → anything marked INSTR), press EDITOR for its own window, and MIDI IN to play it from your keyboard or controller. MIDI IN is OFF until you press it, because keys and MIDI belong to the pads — turn it on to play the synth, off to go back to the pads. Its audio lands on that channel, so the fader, the inserts after it and the console all apply.' },

      { p: 'A KNOB ON YOUR CONTROLLER, ON A PLUGIN\'S OWN PARAMETER (Terminator 3, desktop): a plugin\'s knobs live in ITS window, so Terminator cannot put them on the mixer — but it can still hand them to your hardware. Press MIDI on the plugin\'s insert slot and it lists everything that plugin exposes; RIGHT-CLICK the one you want, choose MIDI Learn, and move a knob or fader on your controller. From then on that control drives it, live, and the binding is remembered with the rest of your MIDI map. The row shows the CC it caught; right-click again to clear it. Nothing else changes — the plugin\'s own window still opens with EDITOR, and everything you do there still works.' },
      { p: 'THE MENU (Terminator 3, desktop): File has New (⌘N), Open (⌘O), Open Recent, Save (⌘S), Save As (⇧⌘S) and Export (⌘E); Transport has Play / Stop; View has Rearrange and Reset Layout; Help opens this book (F1). Preferences is ⌘, in the Terminator menu on a Mac, and under File on Windows. Every one of them does exactly what the button or the key in the app does — one set of controls, two ways in.' },
      { p: 'YOUR OWN PLUGINS (Terminator 3, desktop) — Preferences → PLUGINS. SCAN FOR NEW walks the usual VST3 and Audio Unit folders (ADD FOLDER… adds any other place you keep them) and lists what it finds. Every plugin is opened in its OWN process while it is being scanned, so one that crashes or hangs cannot take Terminator down with it — it lands under DID NOT LOAD instead, and TRY THESE AGAIN (or RESCAN EVERYTHING) gives it another go after you have updated it. RESCAN EVERYTHING re-reads plugins it already knows; SCAN FOR NEW skips them, which is why it is quick.' },
